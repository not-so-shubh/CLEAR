...
from clear_market.oracle import OracleAllocationStatus, compute_oracle_allocation
...
def test_compute_oracle_allocation_rejects_non_admission_state() -> None:
    with pytest.raises(TypeError):
        compute_oracle_allocation(object())
...
def test_zero_accepted_bids_is_explicitly_infeasible() -> None:
    ...
    allocation = compute_oracle_allocation(state)
...
def test_five_seller_golden_vector() -> None:
    policy = _policy()
    state = AdmissionState(policy)
...
def test_equal_lowest_tie_uses_merchant_rank_and_second_ranked_bid() -> None:
...
def test_capacity_ineligible_bid_does_not_affect_second_ranked_payment() -> None:
    state = AdmissionState(_policy())
...
def test_above_reserve_bid_does_not_affect_second_ranked_payment() -> None:
    state = AdmissionState(_policy())
...
def test_accepted_bids_with_no_eligible_bid_are_infeasible() -> None:
    state = AdmissionState(_policy())
...
def test_rejected_cheap_bid_is_absent_from_oracle_result() -> None:
    policy = _policy()
    state = AdmissionState(policy)
...
def test_oracle_is_deterministic_and_does_not_mutate_state() -> None:
    state = AdmissionState(_policy())
    _admit(state, 1, unit_price_paise=110, quantity_available=4)
    _admit(state, 0, unit_price_paise=100, quantity_available=4)
    before = state.accepted_decisions

    first = compute_oracle_allocation(state)
    second = compute_oracle_allocation(state)

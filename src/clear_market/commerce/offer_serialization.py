...
def canonical_merchant_offer_v2_bytes(value: MerchantOfferV2) -> bytes:
    """Serialize every market-facing offer field through an explicit projection."""
    if type(value) is not MerchantOfferV2:
        raise TypeError("value must be exactly a MerchantOfferV2")
    return canonical_json_bytes(
        {
            ...
            "payload": {
                ...
                "inventory_snapshot_commitment_version": (
                    value.inventory_snapshot_commitment_version
                ),
                "inventory_snapshot_commitment_sha256": (
                    value.inventory_snapshot_commitment_sha256
                ),
